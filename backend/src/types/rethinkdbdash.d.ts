declare module 'rethinkdbdash' {
  export interface ConnectionOptions {
    ...
    discovery?: boolean // When true, the driver will regularly pull data from the
    // table server_status to keep a list of updated hosts, default false
    ...
    maxExponent?: number // The maximum timeout before trying to reconnect is 2^maxExponent x timeoutError,
    // default 6(~60 seconds for the longest wait)
    silent?: boolean // console.error errors, default false
    servers?: Array<{ host: string; port: number }> // an array of objects representing RethinkDB nodes to connect to
    optionalRun?: boolean // if false, yielding a query will not run it, default true
    log?: false | ((message: string) => void) // will be called with the log events by the pool master, default false
  }
  ...
  > = ({ [P in L1]: P } & { [P in L2]: never } & Record<string, never>)[L1]

  type ObjectOmit<O, K extends string | number | symbol> = Pick<
    O,
    StringOmit<keyof O, K>
  >

  type ObjectOptional<O, K extends keyof O> = ObjectOmit<O, K> &
    Partial<Pick<O, K>>
  ...
  interface ShardStatusChange {
    primary_replicas: string[]
    replicas: Array<{
      server: string
      state: 'initializing' | 'ready' | 'rebalancing'
    }>
  }
  ...
  interface ReconfigureReturn {
    reconfigured: number
    config_changes: Array<ChangeEvent<RConfig>>
    status_changes: Array<ChangeEvent<StatusChange>>
  }

  interface RethinkDBDash extends RDB {
    connect(obj: ConnectOptions): ConnectOptions
    db(name: string): RDB
    dbList(): RArray<string>
    dbCreate(name: string): RDatum<object>
    expr(obj: any): RDatum<any>
    point(
      longitude: number | RDatum<number>,
      latitude: number | RDatum<number>,
    ): RPoint
    circle(
      point: number[] | RArray<number> | RPoint,
      radius: number | RDatum<number>,
    ): RGeometry
    distance(
      point1: RPoint,
      point2: RPoint,
    ...
    range(
      initial: number | RDatum<number>,
      count: number | RDatum<number>,
    ): RStream<number>
    epochTime(epoch: number | RDatum<number>): RDatum<Date>
    time(
      year: number | RDatum<number>,
      month: number | RDatum<number>,
      day: number | RDatum<number>,
      tz: string | RDatum<string>,
    ): RDatum<Date>
    time(
      year: number | RDatum<number>,
      month: number | RDatum<number>,
      day: number | RDatum<number>,
      hour: number | RDatum<number>,
      minute: number | RDatum<number>,
      second: number | RDatum<number>,
      tz: string | RDatum<string>,
    ): RDatum<Date>
  ...
  interface RDB {
    table<T extends BaseDocument>(name: string | RDatum<string>): RTable<T, any>
    table(name: 'devices'): RTable<DBDevice, any>
    table(name: 'api_calls'): RTable<DBApiCall, any>
    tableList(): RArray<string>
    tableDrop(name: string): RDatum<{
      tables_dropped: 1
      config_changes: { old_val: R_TableConfig; new_val: null }
    }>
    tableCreate(
      name: string,
      opts?: R_TableCreateOptions,
    ): RDatum<{
      tables_created: 1
      config_changes: { old_val: null; new_val: R_TableConfig }
    }>
  }
  ...
  type RDatum<T> = RStreamOrDatum<T> &
    T & {
      reduce(func: (a: RDatum<any>, b: RDatum<any>) => any): RDatum<any>
      run<R extends RunParams | undefined>(
        a?: R,
      ): Promise<R extends { cursor: true } ? Cursor<T> : T>
      ...
      map<U>(
        other: RArray<U> | RStream<U>,
        func: (e: RDatum<any>, x: RDatum<U>) => any,
      ): RArray<any>
      map(
        other: any,
        func: (e: RDatum<any>, x: RDatum<any>) => any,
      ): RArray<any>
      ...
      filter(criteria: (obj: T) => any): RDatum<T>
      filter(obj: T): RDatum<T>
      contains(
        obj: T extends Array<infer El> ? RDatum<El> | El : never,
      ): RDatum<boolean>
      contains(
        func: T extends Array<infer El> ? (e: RDatum<El>) => any : never,
      ): RDatum<boolean>
      ...
      groupBy(
        func: string | ((obj: RDatum<any>) => any),
        transform: (obj: RArray<any>) => any,
      ): RArray<{ group: any; reduction: any }>
      forEach(func: (e: RDatum<any>) => any): RDatum<object>

      fold(
        base: any,
        func: (acc: RDatum<any>, row: RDatum<any>) => any,
        options?: {
          emit: (
            state: RDatum<any>,
            row: RDatum<any>,
      ...
      hasFields(fields: string[]): RDatum<T>
      hasFields(field: string): RDatum<T>
      ...
      deleteAt(
        offset: number | RDatum<number>,
        endOffset?: number | RDatum<number>,
      ): RDatum<any>
      insertAt(
        offset: number | RDatum<number>,
        value: RDatum<any> | any,
      ): RArray<any>
  ...
  export interface RArray<T> extends RDatum<T[]> {
    ...
    map<U>(
      other: RArray<U> | RStream<U>,
      func: (e: RDatum<T>, x: RDatum<U>) => any,
    ): RArray<any>
    ...
    orderBy(
      field: keyof T | R_Sorting<keyof T> | ((e: RDatum<T>) => any),
    ): RArray<T>
    ...
    groupBy(
      func: string | ((obj: RDatum<T>) => any),
      transform: (obj: RArray<T>) => any,
    ): RArray<{ group: any; reduction: any }>
    forEach(func: (e: RDatum<T>) => any): RDatum<object>

    slice(
      startOffset: number | RDatum<number>,
      endOffset: number | RDatum<number>,
    ): RArray<T>
    deleteAt(
      offset: number | RDatum<number>,
    ...
    typeOf(): RDatum<
      'ARRAY' | 'GROUPED_STREAM' | 'GROUPED_DATA' | 'SELECTION<ARRAY>'
    >
  }

  interface RStream<T, IndexNames extends string = never>
    extends RStreamOrDatum<T> {
    run<R extends RunParams | undefined>(
      a?: R,
    ): PromiseLike<R extends { cursor: true } ? Cursor<T> : T[]>
    (idx: number): RDatum<T>
    ...
    update<Opts extends R_UpdateOptions & { returnChanges: true | 'always' }>(
      obj: (obj: RDatum<T>) => any,
      options: Opts,
    ): RDatum<R_UpdateResult & { changes: Array<{ new_val: T; old_val: T }> }>
    update(
      obj: (obj: RDatum<T>) => any,
      options?: R_UpdateOptions,
    ): RDatum<R_UpdateResult>
    update<Opts extends R_UpdateOptions & { returnChanges: true | 'always' }>(
      obj: RUpdateObj<T>,
      options: Opts,
    ): RDatum<R_UpdateResult & { changes: Array<{ new_val: T; old_val: T }> }>
    ...
    isEmpty(): RDatum<boolean>
    pluck(...field: Array<keyof T>): RStream<any>
    delete(): RDatum<R_DeleteResult>
  ...
  export type RethinkUpdateObject<T> = RUpdateObj<T>

  type RUpdateObj<T> = DeepPartial<RDatum<T>>

  export interface RTableSlice<
    T extends BaseDocument,
    IndexNames extends string,
  > extends RStream<T, IndexNames> {
    replace(
      obj: (obj: RDatum<T>) => any,
      options?: R_UpdateOptions,
    ): RDatum<R_InsertResult>
    replace(
      obj: RInsertObj<T>,
      options?: R_UpdateOptions,
    ): RDatum<R_InsertResult>
    ...
    filter(obj: DeepPartial<RDatum<T>>): RTableSlice<T, IndexNames>
    ...
    changes(): RDatum<Promise<Cursor<ChangeEvent<T>>>>
  }

  interface RTableRow<T extends BaseDocument> extends RDatum<T | null> {
    update<Opts extends R_UpdateOptions & { returnChanges: true | 'always' }>(
      obj: (obj: RDatum<T>) => any,
      options: Opts,
    ): RDatum<R_UpdateResult & { changes: Array<{ new_val: T; old_val: T }> }>
    ...
    replace<Opts extends R_UpdateOptions & { returnChanges: true | 'always' }>(
      obj: (obj: RDatum<T>) => any,
      options?: Opts,
    ): RDatum<R_InsertResult & { changes: Array<{ new_val: T; old_val: T }> }>
    replace(
      obj: (obj: RDatum<T>) => any,
      options?: R_UpdateOptions,
    ): RDatum<R_InsertResult>
    ...
    gt(other: T | RDatum<T> | null): RDatum<boolean>
  ...
  interface R_InsertOptions<T extends BaseDocument> {
    ...
    conflict?:
      ...
      | 'replace'
      | 'update'
      | ((id: string, oldDoc: RDatum<T>, newDoc: RDatum<T>) => any)
  }
  ...
  type RInsertObjInternal<T> =
    | RDatum<T>
    | RStream<T>
    | RDatum<T[]>
    | Array<RDatumfy<T>>
    | (() => RInsertObjInternal<T>)
    | RDatumfy<T>
  type RInsertObj<T extends BaseDocument> = RInsertObjInternal<
    ObjectOptional<T, 'id'>
  >
  ...
  > extends RTableSlice<T, IndexNames> {
    config(): RConfig
    reconfigure(options: ReconfigureOptions): string
    get(id: any): RTableRow<T>
    insert<
      Opts extends R_InsertOptions<T> & { returnChanges: true | 'always' },
    >(
      obj: RInsertObj<DeepPartial<T>>,
      options: Opts,
    ): RDatum<R_InsertResult & { changes: Array<{ new_val: T; old_val: T }> }>
    insert(
      obj: RInsertObj<DeepPartial<T>>,
      options?: R_InsertOptions<T>,
    ): RDatum<R_InsertResult>
    changes(): RethinkChangefeedHandler | RethinkChangefeedErrorHandler
    indexList(): RArray<string>
    indexCreate(
      name: IndexNames,
      func: (obj: RDatum<T>) => any,
      opts?: { multi?: boolean; geo?: boolean },
    ): RDatum<{ created: 1 }>
    indexCreate(
      name: keyof T & IndexNames,
      opts?: { multi?: boolean; geo?: boolean },
    ): RDatum<{ created: 1 }>
    indexCreate(
      name: keyof T & IndexNames,
    ...
    // TODO Not sure why this had to be overridden rather than id being rest args, should try later
    getAll(id: any, opts?: { index: IndexNames }): RTableSlice<T, IndexNames>
    getAll(
      id1: any,
      id2: any,
      opts?: { index: IndexNames },
    ): RTableSlice<T, IndexNames>
    getAll(
      id1: any,
      id2: any,
      id3: any,
    ...
    filter(
      criteria: (obj: DeepPartial<RDatum<T>>) => any,
    ): RTableSlice<T, IndexNames>
    filter(
      obj: DeepPartial<RDatumfy<T>>,
  ...
  export type RethinkChangefeed<T> = () => Promise<
    Promise<Cursor<ChangeEvent<T>>>
  >
  export type RethinkChangefeedHandler<T> = (
    event: ChangeEvent<T>,
  ) => Promise<void> | void
  export type RethinkChangefeedErrorHandler = (error: Error) => void

  export default function (options: ConnectionOptions): RethinkDBDash
}
