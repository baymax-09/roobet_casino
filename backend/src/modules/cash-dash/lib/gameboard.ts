...
import {
  type ActiveCashDashGame,
  type Deck,
} from '../documents/active_cash_dash_games'

const DEFAULT_ROWS = config.cashdash.defaultRows
...
export function getGameboardLayout(difficulty: Difficulty, amount: number) {
  ...
  let maxBet = config.cashdash.maxBet
...
export function getCurrentRow(activeGame: ActiveCashDashGame): number {
