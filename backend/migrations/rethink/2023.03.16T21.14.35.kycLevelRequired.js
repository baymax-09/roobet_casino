...
exports.up = async ({ context: { r, rethinkConnection, db } }) => {
  ...
  cursor.each(async (error, row) => {
    ...
    if (row['userKYCOverrideLevel'] !== undefined) {
      await User.get(row.id).update({ kycRequiredLevel: row['userKYCOverrideLevel'] }).run(rethinkConnection)
    }
  })
}
...
exports.down = async ({ context: { r, rethinkConnection, db } }) => {
  ...
  cursor.each(async (error, row) => {
    ...
    await User.get(row.id).replace(r.row.without('kycRequiredLevel')).run(rethinkConnection)
  })
}
